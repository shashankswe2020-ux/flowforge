"""Claude Code adapter — request/response normalization for Claude Code."""
...
class ClaudeCodeAdapter(AdapterBase):
    """Adapter for Anthropic Claude Code integration.

    Normalizes Claude Code-specific input format (sessionId, message, project)
    into the canonical schema and maps graph output back.
    """

    adapter_id: str = "claude_code"
    auth_mode: str = "api_key"

    def normalize_request(self, raw_input: dict[str, object]) -> CanonicalRequest:
        """Normalize Claude Code input to canonical request."""
        project_obj = raw_input.get("project", {})
        repo_context = ""
        if isinstance(project_obj, dict):
            repo_context = str(project_obj.get("path", ""))
        ...
        context_raw = raw_input.get("context", {})
        metadata: dict[str, object] = {}
        if isinstance(context_raw, dict):
            metadata = dict(context_raw)

        return CanonicalRequest(
            request_id=str(raw_input.get("sessionId", "")),
            assistant_provider="claude_code",
            user_prompt=str(raw_input.get("message", "")),
    ...
    def map_error(self, error: Exception) -> IntegrationError:
        """Map exception to Claude Code-specific IntegrationError."""
